...
# Sử dụng thư viện regex để kiểm tra IP address
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            uncomment_cfg_dir_ubuntu_windows('/usr/local/nagios/etc/nagios.cfg')

            # Xử lý máy chủ Ubuntu
            host_name, alias, contact_group, ip_address, file_name = input_server_info_linux()

            # gọi hàm
            create_host_config_ubuntu(host_name, alias, contact_group, ip_address, file_name)
            ...
            host_name, alias, contact_group, ip_address, file_name = input_server_info_windows()
            # gọi hàm
            create_host_config_windows(host_name, alias, contact_group, ip_address, file_name)
            ...
            host_name, alias, contact_group, ip_address, file_name, oid_port_in, oid_port_out = input_server_info_router()
            # gọi hàm
            create_host_config_router(host_name, alias, contact_group, ip_address, file_name, oid_port_in, oid_port_out)
            ...
            host_name, alias, contact_group, ip_address, file_name = input_server_info_windows()
            # gọi hàm
            create_host_config_windows_server(host_name, alias, contact_group, ip_address, file_name)
...
# Hàm để tạo và điền thông tin vào tệp cấu Shình Ubuntu
...
def is_valid_group(hostname):
    # Kiểm tra host_name và alias không chứa ký tự đặc biệt (trừ "/") và không rỗng
...
def is_valid_filename(filename):
    # Kiểm tra file_name không chứa khoảng trắng và không để trống
    return filename and not any(char.isspace() for char in filename)


def get_valid_input_option(input_str):
    # Kiểm tra xem đầu vào có chỉ chứa ký tự chữ và dấu phẩy hay không
...
def is_valid_not_check_space(hostname):
    # Kiểm tra host_name và alias không chứa ký tự đặc biệt
...
def check_duplicate_ips_in_cfg_files(ip_address):
    ...
    try:
        for directory in directories:
            existing_ip_addresses.update(get_existing_ip_addresses(directory))
...
def execute_ssh_commands_ubuntu(ip_address, username, password, ip_to_add, commands):
    ...
    # Biến cờ để kiểm tra xem đã thêm dòng "command[check_swap]" chưa
    ...
    try:
        ...
        for command in tqdm(commands_check_nscp, desc="CHECKING ...", unit=""):
            ...
            result += output + "\n"  # Append output to result variable
        ...
        if found_active:
            ...
            # Thực hiện từng lệnh CMD trong danh sách
            for cmd in tqdm(commands, desc="DOWNLOADING NRPE", unit=""):
                ...
                time.sleep(1)  # Đợi cho lệnh hoàn thành

            # Thêm thời gian chờ sau khi cài đặt
            time.sleep(5)

            # Cấp quyền ghi cho tệp cấu hình
            cmd_chmod = f'echo {password} | sudo -S chmod 777 /etc/nagios/nrpe.cfg'
            stdin, stdout, stderr = ssh.exec_command(cmd_chmod)
            time.sleep(1)  # Đợi cho lệnh hoàn thành

            # Tạo một tệp cấu hình mới với các thay đổi
            new_config = []
            with ssh.open_sftp() as sftp:
                ...
                with sftp.file(remote_file_path, 'r') as remote_file:
                    for line in remote_file:
                        ...
                        if line.startswith('command[check_hda1]=/usr/lib/nagios/plugins/check_disk'):
                            ...
                            # Kiểm tra xem đã thêm "command[check_swap]" chưa
                ...
                # Ghi lại tệp cấu hình mới
                with sftp.file(remote_file_path, 'w') as remote_file:
                    remote_file.writelines(new_config)
                    remote_file.flush()
                    remote_file.close()

            # Cuối cùng, khởi động lại dịch vụ nagios-nrpe-server
            # Thực hiện lệnh để thêm địa chỉ IP vào tệp cấu hình
        ...
        print(Fore.RED + f"\nUNABLE TO CONNECT TO CLIENT {ip_address}" + Style.RESET_ALL)
    except Exception as e:
        if not connected:  # Nếu chưa kết nối, hiển thị thông báo lỗi
            print(Fore.RED + f"\nINSTALLATION FAILED - UNABLE TO CONNECT TO CLIENT {ip_address}" + Style.RESET_ALL)
        else:
...
def input_server_info_linux():
    ...
    while not is_valid_hostname(alias):
    ...
    print(" ")
    while not is_valid_group(contact_group):
    ...
    print(" ")
    while check_duplicate_ips_in_cfg_files(ip_address):
    ...
    while not is_valid_ip(ip_address):
        ...
        # Thực hiện cài đặt NRPE nếu người dùng chọn "Chưa."
    print(Fore.CYAN + "\n###################################################")
    print("### CREATE TEST SESSION AND INSTALL NRPE CLIENT ###")
    print("###################################################\n" + Style.RESET_ALL)
    username = input("Enter username: ")
    while not is_valid_filename(username):
    ...
    while not is_valid_ip(ip_to_add):
        print("The IP address is not properly formatted and not enter IP DNS. Please re-enter.")
        print("Class A: 1.0.0.0 - 126.255.255.255\n"
              "Class B: 128.0.0.0 - 191.255.255.255\n"
              "Class C: 192.0.0.0 - 223.255.255.255\n")
        ip_to_add = input("Enter IP nagios server: ")
        print(" ")
    host_name = execute_ssh_commands_ubuntu(ip_address, username, password, ip_to_add, commands)
    ...
    if host_name is not None:
        ...
        while os.path.exists(path) or check_existing_filename_servers(file_name):
        ...
        while not is_valid_filename(file_name):
        ...
        return input_server_info_linux()
...
def input_server_info_windows():
    ...
    while not is_valid_ip(nagios_server_ip):
        print("The IP address is not properly formatted and not enter IP DNS. Please re-enter.")
        print("Class A: 1.0.0.0 - 126.255.255.255\n"
              "Class B: 128.0.0.0 - 191.255.255.255\n"
              "Class C: 192.0.0.0 - 223.255.255.255\n")
        nagios_server_ip = input("Enter IP nagios server: ")
        print(" ")
    host_name = execute_ssh_commands_windows(username, password, ip_address, nagios_server_ip)
    ...
    if host_name is not None:
        ...
        return input_server_info_windows()
...
def input_server_info_router():
    ...
    while not check_oid(oid_port_in):
    ...
    while not check_oid(oid_port_out):
        print("The OID is invalid. Please re-enter.")
        print("Does not contain special characters and is not empty \n")
        oid_port_out = input("Enter the OID parameter <Port 1 OUT Bandwidth Usage> [DEFAULT: 1.3.6.1.2.1.2.2.1.10.8]: ")
        if not oid_port_out:
